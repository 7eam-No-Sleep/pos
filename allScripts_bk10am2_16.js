...
function fetchCustomerDetails() {
    let phoneNumber;
    phoneNumber = document.getElementById('customerSelection').value;
...
function displayInventory(inventoryData) {
    ...
    inventoryData.forEach(product => {
        ...
        productElement.innerHTML = `
            ...
            <button class="print-barcode-button" data-product-id="${product.ProductID}">Print Barcode</button>
            <hr>
        `;
...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        row.innerHTML = `
            ...
            <td><button class="edit-button" data-product-id="${product.ProductID}">Edit</button></td>
            <td><button class="print-barcode-button" data-product-id="${product.ProductID}">Print Barcode</button></td>
        `;
...
// Update the function to handle click events for the edit button
function setupEditButton() {
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            window.location.href = `edit_product/edit_product.php?productID=${productID}`;
        });
    });
}

// Add this function to setup event listeners for "Print Barcode" buttons
function setupPrintBarcodeButton() {
    document.querySelectorAll('.print-barcode-button').forEach(button => {
        button.addEventListener('click', () => {
            const productId = button.getAttribute('data-product-id');
            console.log('Product ID:', productId); // Log product ID to the console for debugging

            // Retrieve item description directly from the product object
            const itemDescription = product.Description;
            console.log('Item Description:', itemDescription); // Log item description to the console for debugging

            // Redirect to the barcode preview page with product ID and item description appended to the URL
            window.location.href = `barcode-preview.html?productId=${productId}&description=${encodeURIComponent(itemDescription)}`;
        });
    });
}
